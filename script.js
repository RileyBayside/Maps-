...
(function(){
  ...
  const CUSTOM_ORDERS = Object.fromEntries(Object.entries(CUSTOM_ORDER_STRINGS).map(([k,v]) => [k, v.trim().split(/\s+/).filter(Boolean)]));
  ...
  function orderByCustom(items, zoneKey){
    ...
    if (seq && seq.length) {
      ...
      return seq.map(id=>map[id]).filter(Boolean);
    }
    return items;
  }
  ...
  function parseField(desc, key){
    if(!desc) return '';
    const re = new RegExp(key.replace(/[-/\\^$*+?.()|[\]{}]/g,'\\$&')+'.*?<td[^>]*>(.*?)<\\/td>', 'i');
  ...
  function renderZones(data){
    ...
    L.geoJSON(data, {
      ...
      onEachFeature: function(feature, layer){
        ...
        const name = parseField(desc,'Name') || '';
    ...
    keys.forEach(k=>{
      ...
      ordered.forEach((it, idx)=>{
        ...
        // Popup numbering to match sidebar
