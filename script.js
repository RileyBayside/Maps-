...
// ====== Parks Sidebar with custom orders + admin-only controls ======
(function(){
...
"M0401",],
  ...
  function ensureSidebar() {
    ...
    if (!side) {
      ...
      side.id = 'sidebar';
      side.innerHTML = '<h2>Parks</h2><div class="admin-actions"><button id="adminLoginBtn">Admin Login</button></div><div id="catchmentList"></div>';
  ...
  function renderZones(data){
    ...
    L.geoJSON(data,{
      style:function(feature){
        ...
        let cpy=0; if(desc){ const match=desc.match(/CutsPerYear.*?<td[^>]*>(\d+)<\/td>/); if(match) cpy=parseInt(match[1]); }
        let color='#808080'; if(cpy===18) color='#1f77b4'; else if(cpy===15) color='#2ca02c';
        return {color,weight:2,fillOpacity:0.4};
      },
      onEachFeature:function(feature,lyr){
        const desc=feature.properties&&feature.properties.description&&feature.properties.description.value;
        let mowingID=feature.properties.name||'Unknown'; if(desc){ const m=desc.match(/MowingID.*?<td[^>]*>(.*?)<\/td>/); if(m) mowingID=m[1]; }
        let name=''; if(desc){ const n=desc.match(/Name.*?<td[^>]*>(.*?)<\/td>/); if(n) name=n[1]; }
        let catchment='Unassigned'; if(desc){ const c=desc.match(/OSS_CATCHMENT.*?<td[^>]*>(.*?)<\/td>/); if(c) catchment=c[1]; }
  ...
  function buildCsv(){
    ...
    document.querySelectorAll('.catchment-section').forEach(section=>{
      const zone=section.querySelector('.catchment-title').innerText;
      section.querySelectorAll('.park-list li').forEach((li,idx)=>{
        ...
        const name=span.innerText.split(' – ')[1]||'';
        ...
        rows.push([zone,idx+1,idLabel,name,checked,note.value.replace(/[\r\n]+/g,' '),st.lastUpdated||'']);
      });
    });
  ...
  function showAdminControls(){
    ...
    actions.innerHTML='<button id="exportCsvBtn">Export CSV</button><button id="resetBtn">Reset All</button>';
    document.getElementById('exportCsvBtn').addEventListener('click',()=>{
      const csv=buildCsv(); const blob=new Blob([csv],{type:'text/csv'});
      const url=URL.createObjectURL(blob); const a=document.createElement('a');
      a.href=url; a.download='parks_status.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
    });
  ...
  fetch('data.geojson').then(r=>r.json()).then(data=>{ renderZones(data); });
})();
// ====== End Parks Sidebar ======
