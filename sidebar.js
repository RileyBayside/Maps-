...
function showToday() {
  ...
  let html = "<html><head><title>Completed Today</title></head><body>";
  html += "<h2>Completed Parks for " + today + "</h2>";
  html += "<table border='1' cellspacing='0' cellpadding='5'><tr><th>Park ID</th><th>Name</th><th>Note</th><th>Time</th></tr>";
  for (const park in parkData) {
    if (parkData[park].done && parkData[park].time.startsWith(today)) {
      html += "<tr>";
      html += "<td>" + park + "</td>";
      html += "<td>" + (parkNames[park] || "") + "</td>";
      html += "<td>" + (parkData[park].note || "") + "</td>";
      html += "<td>" + parkData[park].time + "</td>";
      html += "</tr>";
    }
  }
...
function exportPDF() {
  const today = new Date().toISOString().slice(0,10);
  let html = "<html><head><title>Export Parks Data</title></head><body>";
  html += "<h2>Parks Data Export (" + today + ")</h2>";
  html += "<table border='1' cellspacing='0' cellpadding='5'><tr><th>Park ID</th><th>Name</th><th>Done</th><th>Note</th><th>Time</th></tr>";
  for (const park in parkData) {
    const d = parkData[park];
    html += "<tr>";
    html += "<td>" + park + "</td>";
    html += "<td>" + (parkNames[park] || "") + "</td>";
    html += "<td>" + (d.done ? "Yes" : "No") + "</td>";
    html += "<td>" + (d.note || "") + "</td>";
    html += "<td>" + (d.time || "") + "</td>";
    html += "</tr>";
  }
  ...
  win.document.write(html);
  win.document.close();
  win.print(); // user can choose "Save as PDF" in browser
}
