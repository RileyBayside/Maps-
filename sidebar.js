...
/* ===== Bayside Sidebar (clean build) =====
 * Features:
 * - Collapsible left sidebar positioned below map controls
 * - Zones 1-5 with custom order (IDs only; names pulled dynamically from GeoJSON)
 * - Each park: checkbox (done), notes (text) with localStorage persistence
 * - Click ID/Name to zoom to park on map
 * - Admin login (password: Fishing101!) unlocks tools:
 *     Export CSV, Show Completed Today (printable), Export to PDF, Clear All User Data
 * - Sidebar open/closed state persists in localStorage
 * - Auto index of park names from existing GeoJSON layers on the map
 */
(function() {
  // ---- Config ----
  ...
  // ---- Zones (IDs only; names loaded dynamically) ----
  const zones = {
  ...
  let parkNames = {}; // filled from GeoJSON
  ...
  // ---- Build Name Index by scanning map layers ----
  function buildNameIndexFromMap() {
    if (!window.map || typeof window.map.eachLayer !== "function") return 0;
    let count = 0;
    window.map.eachLayer(function(layer) {
      // check for GeoJSON layers
      if (layer && typeof layer.eachLayer === "function" && layer.getLayers) {
        try {
          layer.eachLayer(function(featLayer) {
            ...
            if (f && f.properties) {
              ...
              const name = f.properties.Name || f.properties.NAME || f.properties.title;
              if (id) {
                if (!parkNames[id]) count++;
                parkNames[id] = name || "";
              }
            }
          });
        } catch(e) { /* ignore non-geojson layers */ }
      }
    });
    return count;
  }

  // Poll until we index names (max ~10s)
  function waitForNameIndexThenRender() {
    const start = Date.now();
    const interval = setInterval(() => {
      const added = buildNameIndexFromMap();
      if (added > 0) {
        nameIndexReady = true;
        ...
        clearInterval(interval);
        renderSidebar(); // render anyway with IDs only
      }
    }, 800);
  }
        ...
        try {
          layer.eachLayer(function(featLayer) {
            ...
            if (f && f.properties) {
              const id = f.properties.MowingID || f.properties.UniqueID || f.properties.ID;
              if (id === parkId) {
                // Use feature bounds; fallback to setView if no bounds
                if (typeof featLayer.getBounds === "function") {
                  window.map.fitBounds(featLayer.getBounds());
                } else if (typeof featLayer.getLatLng === "function") {
                  window.map.setView(featLayer.getLatLng(), 18);
                }
                found = true;
              }
            }
  ...
  window.exportData = function() {
    let csv = "Park,Name,Done,Note,Time\n";
    const ids = new Set();
    // ensure consistent order by zones
    Object.values(zones).flat().forEach(id => ids.add(id));
    ids.forEach(park => {
      const d = parkData[park] || {};
      csv += `${park},${(parkNames[park]||"").replace(/,/g,";")},${d.done? "Yes":"No"},${(d.note||"").replace(/,/g,";")},${d.time||""}\n`;
    });
  ...
  window.showToday = function() {
    ...
    let html = "<html><head><title>Completed Today</title><style>body{font-family:sans-serif} table{border-collapse:collapse;width:100%} th,td{border:1px solid #ccc;padding:6px;text-align:left} th{background:#f5f5f5}</style></head><body>";
    html += "<h2>Completed Parks for " + today + "</h2>";
    html += "<table><tr><th>Park ID</th><th>Name</th><th>Note</th><th>Time</th></tr>";
    Object.keys(parkData).forEach(park => {
      const d = parkData[park];
      if (d.done && d.time && d.time.startsWith(today)) {
        html += "<tr><td>"+park+"</td><td>"+(parkNames[park]||"")+"</td><td>"+(d.note||"")+"</td><td>"+d.time+"</td></tr>";
      }
    });
    html += "</table></body></html>";
    const win = window.open("", "CompletedToday");
  ...
  window.exportPDF = function() {
    const today = new Date().toISOString().slice(0,10);
    let html = "<html><head><title>Parks Data Export</title><style>body{font-family:sans-serif} table{border-collapse:collapse;width:100%} th,td{border:1px solid #ccc;padding:6px;text-align:left} th{background:#f5f5f5}</style></head><body>";
    html += "<h2>Parks Data Export (" + today + ")</h2>";
    html += "<table><tr><th>Park ID</th><th>Name</th><th>Done</th><th>Note</th><th>Time</th></tr>";
    const ids = new Set();
    Object.values(zones).flat().forEach(id => ids.add(id));
    ids.forEach(park => {
      const d = parkData[park] || {};
      html += "<tr><td>"+park+"</td><td>"+(parkNames[park]||"")+"</td><td>"+(d.done? "Yes":"No")+"</td><td>"+(d.note||"")+"</td><td>"+(d.time||"")+"</td></tr>";
    });
    html += "</table></body></html>";
    const win = window.open("", "ExportPDF");
    win.document.write(html);
    win.document.close();
  ...
  window.clearUserData = function() {
    if (confirm("Are you sure you want to clear ALL user data? This cannot be undone.")) {
      localStorage.removeItem(PARK_DATA_KEY);
      parkData = {};
      renderSidebar();
