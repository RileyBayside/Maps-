...
(function() {
  ...
  let parkNames = {}; // MowingID -> Name

  function saveData() {
    localStorage.setItem(PARK_DATA_KEY, JSON.stringify(parkData));
  }

  // Attempt to extract a field from properties or embedded HTML in description
  function extractField(props, field) {
    if (!props) return "";
    const direct = props[field] || props[field.toLowerCase()] || props[field.toUpperCase()];
    if (direct) return direct;
    const desc = (props.description && (props.description.value || props.description)) || "";
        ...
        try {
          layer.eachLayer(function(featLayer) {
            ...
            if (f && f.properties) {
              ...
              const name = extractField(f.properties, "Name") || f.properties.name || "";
              if (id) parkNames[id] = name || "";
            }
          });
        } catch (e) { /* ignore */ }
      }
  ...
  // Admin functions kept minimal here (loaded elsewhere previously)
